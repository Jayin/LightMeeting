...
function getpagestr($pagenow,$topage,$allcount,$count){
	...
	$str='';
	//$count=count($text);
	$pagecount=ceil($allcount/$count);
	if ($pagenow>1) {
		$str.="<a href='".$topage."?pagenow=".($pagenow-1)."'>上一页</a>   ";
	}
	for($i=1;$i<10 && $i<=$pagecount;$i++){
		$str.="<a href='".$topage."?pagenow=".$i."'>".$i."</a>   ";
	}
	if($pagenow<=$pagecount){
		$str.="<a href='".$topage."?pagenow=".($pagenow+1)."'>下一页</a>   ";
	}


	$str.="当前页面".$pagenow."/".$pagecount."   共".$allcount."记录";
	return $str;
}
